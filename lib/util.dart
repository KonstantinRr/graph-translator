...
class UID {
  ...
  factory UID.gen([
    ...
    var idx = 0;
    Uint8List data =
        Uint8List(componentLength * componentCount + componentCount - 1);
    for (var count = 0; count < componentCount; count++) {
      for (var length = 0; length < componentLength; length++)
        data[idx++] =
            components[random.nextInt(components.length)].codeUnitAt(0);
      if (count < componentCount - 1) data[idx++] = 45;
    }
  ...
  @override
  bool operator ==(o) {
    if (o is UID) return o.data == data;
    if (o is String) return o == data;
    return false;
  }
